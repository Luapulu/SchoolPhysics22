...
# ╔═╡ e248d08f-fccd-42f2-80b3-e4e37fe77b86
using CairoMakie

# ╔═╡ fd20a949-7e32-4fce-a470-2f73f676ead5
using BenchmarkTools

# ╔═╡ 8f77ee82-2f8a-44b5-8d9e-245bd9831867
using ForwardDiff

# ╔═╡ 620687a2-a48e-4622-8b2c-d33a91db7606
using IntervalArithmetic

# ╔═╡ 4459251a-f5c4-4c93-8c79-9c83744d273b
using Roots

# ╔═╡ 6707f0c1-261d-45db-88c7-5b01170ef2be
...
# ╔═╡ 7f3f0fb5-9499-4f32-931f-a9b2993f361f
struct Bisection end

# ╔═╡ 6ffe1466-2576-41be-bbbb-93ea51499792
function findroot(
	f, (lb, ub)::Tuple{T, T}, xtol::T, method::Bisection
) where T <: Real
	a, b = lb, ub
	fa, fb = f(a), f(b)
	sign(fa) == zero(T) && return a
	sign(fb) == zero(T) && return b
	while abs(b - a) >= 2xtol
		# c = (a*fb - b*fa) / (fb - fa)
		c = (a + b) / 2
		fc = f(c)
...
# ╔═╡ 34f3c0a7-eb71-4610-9d7d-46decaca881e
struct ITP{T<:Real}
	n0::Int
	k1::T
end

# ╔═╡ 327ab5c9-3451-4312-8aad-bf7b65989f05
function findroot(
	f, (lb, ub)::Tuple{T, T}, xtol::T, (;n0, k1)::ITP{T}
) where T <: Real
	a = lb
	fa = f(a)
	sa = sign(fa)
	sign(a) == zero(T) && return a

	b = ub
	fb = f(b)
	sign(b) == zero(T) && return b
	...
	r0 = xtol * 2^(n0 + ceil(Int, log2(abs(ub - lb) / xtol)) - 1)
	while abs(b - a) > 2xtol
		xm = (a + b) / 2
		xf = (a * fb - b * fa) / (fb - fa)
		...
		sigma = xm - xf
		delta = k1 * abs2(b - a)
		xt = delta <= abs(sigma) ? xf + copysign(delta, sigma) : xm

		r = r0 - abs(b - a) / 2
		xitp = abs(xt - xm) <= r ? xt : xm - copysign(r, sigma)
		fitp = f(xitp)
		sitp = sign(fitp)
		if sitp == one(T)
            b = xitp
...
# ╔═╡ c38797d4-90c5-40d2-8142-604e9fcee8db
struct CounterNewton{N} end

# ╔═╡ e9ed7870-b279-4dad-8b5b-4bc012c47a25
function findroot(
	f, (lb, ub)::Tuple{T, T}, xtol::T, method::CounterNewton{N}
) where {N, T <: Real}
	D(x) = ForwardDiff.derivative(f, x)
	L = ub - lb
	a, b = lb, ub
	fa, fb = f(a), f(b)
	fa == zero(T) && return a  # return early if 0 found
	fb == zero(T) && return b
	for _ in 1:N  # do bisection a few times first
		c = (a + b) / 2
		fc = f(c)
		if sign(fc) == sign(fa)
			a, fa = c, fc
		elseif sign(fc) == sign(fb)
			...
			@assert sign(fc) == zero(T)
			return c
		end
	end
	if abs(fb) < abs(fa)  # switch if b closer than a
		a, b = b, a
		fa, fb = fb, fa
	end
	@assert sign(fa * fb) == -one(T)
	Da = D(a)
	while abs(b - a) > 2xtol
		cn = a - fa / Da
		lo, hi = minmax(a, b)  # if newton candidate in interval, use it
		c = lo < cn < hi ? cn : (a + b) / 2  # else, use bisection
		fc = f(c)
		Da = D(c)
		if sign(fc) == sign(fa)  # replace a
			a = c
			fa = fc
		elseif sign(fc) == sign(fb)  # replace b and switch
			a, b = c, a
			fa, fb = fc, fa
		else
			@assert fc == zero(T)
...
# ╔═╡ 3f217ed8-c39d-4b70-88c9-1d3546d8a4ae
struct IntervalNewton end

# ╔═╡ 8841f6d9-6a09-4fd7-b2ab-107267d20815
function findroot(
	f, (lb, ub)::Tuple{T, T}, xtol::T, method::IntervalNewton
) where T <: Real
	X = interval(lb, ub)
	D(x) = ForwardDiff.derivative(f, x)
	while diam(X) > xtol
        ...
        if NX == X
			s = sign(f(X.lo) * fm.hi) 
			if s == -one(T)
				X = interval(minmax(X.lo, m.hi)...)
			elseif s == one(T)
				X = interval(minmax(m.hi, X.hi)...)
			elseif fm.hi == zero(T)
				return m
			else
				return X.lo
...
# ╔═╡ f8d9a832-7ab5-4a05-9a5c-b37f1c1ed58e
struct CalumSecant end

# ╔═╡ 31782043-158c-4de6-8cdc-7491b6427de1
function findroot(
	f, (lb, ub)::Tuple{T, T}, xtol::T, method::CalumSecant
) where T <: Real
	seed = (lb + ub)/2
	seed1 = seed + 0.0000001
	for i in 1:1000
		fr = f(seed)
		fr1 = f(seed1)
		d = (seed1 * fr - seed * fr1) / (fr - fr1)
		dif = f(seed)/d
		seed1 = seed
		seed = seed - dif
		if seed < lb || seed > ub
		...
		if sign(f(seed)) == sign(f(lb))
			lb = seed
		elseif sign(f(seed)) == sign(f(ub))
			ub = seed
		end
		if isnan(seed)
			seed = (lb + ub) / 2
...
# ╔═╡ cd6274c1-6534-4055-afb0-f67181160ab5
struct DamianTwoThirdSecant end

# ╔═╡ e6100249-1c50-4c1b-a3a9-3206c7be2404
function findroot(
	f, (lb, ub)::Tuple{T, T}, xtol::T, method::DamianTwoThirdSecant
) where T <: Real
	x = find_zero(f, (lb, ub), Order1())
	return x > ub || x < lb ? find_zero(f, (lb, ub)) : x
end

# ╔═╡ 18782c85-eee8-4397-88d1-367102dc884e
struct ChrisBisection end

# ╔═╡ 18209d69-c592-4654-8677-4db0c57a165f
function findroot(
	f, (lb, ub)::Tuple{T, T}, xtol::T, method::ChrisBisection
) where T <: Real
	sign(f(lb)) == sign(f(ub)) && throw(
		ArgumentError("function has the same sign at both ends of interval."))

	md = (lb + ub) / 2
	while abs((ub-lb)/2) > xtol
		sign(f(md)) == sign(f(lb)) ? lb = md : ub = md
		md = (lb + ub) / 2
	end
	return md
end
...
testmethod = DamianTwoThirdSecant()

# ╔═╡ 1f3db068-872b-45c8-8e62-d3e1e13b55cd
test_tol = 1e-11

# ╔═╡ 7e6cc62d-778a-49a8-98e9-f970a584359f
findroot(x -> (x - 4) * (x - 1), (-3.0, 3.0), test_tol, testmethod)

# ╔═╡ 96562840-e187-4efa-8bb9-a4f4ef92edc2
findroot(identity, (-1.0, 3.0), test_tol, testmethod)

# ╔═╡ 952cf622-14ed-48f8-b72a-b07e0db09289
@test abs(findroot(identity, (-1.0, 3.0), test_tol, testmethod)) < test_tol
...
# ╔═╡ 44c74ecf-0b05-4e81-b9ae-0e2446a243f0
@benchmark testbf($0.1)

# ╔═╡ ac28a6f4-bb90-4ec7-a683-9cdf12d6c991
testbfs = rand(BenchFunction{Float64}, 10_000)
...
benchmark(testmethod, testbfs, test_tol)

# ╔═╡ a499d91b-5bf1-400a-a08d-fe78b018f75c
begin
	benchmethods = Dict(
		"Bisection" => Bisection(),
		"ITP" => ITP(4, 0.1),
		"CounterNewton" => CounterNewton{2}(),
		"IntervalNewton" => IntervalNewton(),
		"DamianTwoThirdSecant" => DamianTwoThirdSecant(),
		"ChrisBisection" => ChrisBisection()
	)
	suite = BenchmarkGroup()
	for (k, v) in pairs(benchmethods)
		suite[k] = @benchmarkable benchmark($v, $benchbfs, 1e-15) setup=(shuffle!(benchbfs))
	end
	suite
end

# ╔═╡ d3a5554e-c064-4e8a-9c83-93a66ff5e669
benchresults = run(suite)

# ╔═╡ 472fb499-f71f-4c66-95d0-afeab06bf54b
minbenchresults = minimum(benchresults)

# ╔═╡ a1bd52b6-6199-486f-b07b-02071b22ada0
rand(BenchFunction{Float64})

# ╔═╡ b8f33e5c-413e-492f-95a1-cdb229e027a1
let fig = Figure()
	ax = Axis(fig[1, 1])
	ylims!(ax, (-2.0, 2.0))

	xs = range(-1.0, 1.0, length=1_000)
	for _ in 1:5
		bf = rand(BenchFunction{Float64})
		lines!(ax, xs, bf.(xs))
	end
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "bb0bd6223851042eeb925d5b25783b6ef17b3262"
